...
public class PatientController {
    ...
    public ResponseEntity<Patient> searchById(@PathVariable Integer patientId) throws PatientNotFoundException {
        var p = patientService.searchPatientById(patientId);
        return new ResponseEntity<>(p, HttpStatus.OK);
    }

    @GetMapping("/adsweb/api/v1/patient/search/{name}")
    public ResponseEntity<Patient> searchByLastName(@PathVariable String name) throws PatientNotFoundException {
    ...
    public ResponseEntity<Patient> updatePatient(@PathVariable Integer patientId, @RequestBody Patient patient) throws PatientNotFoundException {
